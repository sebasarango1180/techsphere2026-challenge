...
| **Docker on macOS has no Metal passthrough** | Silent CPU fallback would contradict the "GPU/Metal if available" requirement and hurt latency numbers | Native-host run path for `ollama`/`voice-agent`/`vector-store` on Mac, documented and scripted, not manual. `vector-store` joined this group after finding it was the one service still silently CPU-only (Metal has to actually reach every heavy compute path, not just the LLM) — verified all four with real evidence, not assumed: Ollama's `size_vram` exactly equals its model size (100% GPU-resident); BGE-M3 reports `mps:0` via `get_embedder().target_devices`; Kokoro TTS needed an explicit fix (its own device auto-detection only checks `torch.cuda.is_available()`, no MPS branch at all — silently CPU even natively — fixed by passing `device='mps'` explicitly, 2.6s→1.2s per synthesis call); faster-whisper (local STT) confirmed CPU-only with no fix available (CTranslate2 has no Metal backend), acceptable since `groq` is the default STT mode, not `local` |
| **Moving vector-store natively (for Metal) also silently took Chroma out of Docker** | Chroma was embedded (`PersistentClient`, a local file) rather than run as its own service, so it followed vector-store's process wherever that ran — even though Chroma itself does no GPU/Metal work at all. Made the stack harder to reason about (no `chroma` container visible in `docker compose ps`) for no benefit | Switched to `chromadb.HttpClient` against the official `chromadb/chroma` server image, pinned to the same version as the client library. Chroma now runs in Docker in BOTH modes (tagged "tqida", same as postgres/livekit), decoupled from vector-store's native/Docker split entirely — see docker-compose.yml's top comment and `services/vector-store/README.md` |
...
| **Bulk-loading the full given corpus must complete as part of the timed boot** — overruled an earlier design that backgrounded this (reasoning: a system that can't answer from the knowledge base isn't "corriendo y accesible" yet, regardless of what G2's literal wording permits) | A slow or broken bulk-load now directly threatens G2, not just RAG-quality scoring — real pressure to make full-corpus ingest fast and reliable, not just eventually-consistent | Found and fixed three real bugs blocking this: `/v1/ingest` blocked vector-store's event loop (client concurrency was a no-op); BGE-M3 encode() and ChromaDB's client are both NOT thread-safe under concurrency (found live — corrupted requests, not just slow ones — fixed with locks around each); BGE-M3 was CPU-only even in native-agent mode because vector-store still ran in Docker (Docker Desktop has no Metal passthrough) — moved vector-store to native mode too (alongside ollama/voice-agent). Net effect measured live: Docker/CPU ran ~90s/document; native/Metal runs single-digit seconds/document — see vector-store's README and root README for the exact final corpus timing |
